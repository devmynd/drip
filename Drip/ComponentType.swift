...
public extension ComponentType {
  /**
   Note: Only should be used to implement of custom generators.
  ...
  */
  func resolve<T>(generator: (Self) -> T) -> T {
    return lazyMatchFor(generator)(self)
  }
}

extension ComponentType {
  func lazyMatchFor<T>(generator: (Self) -> T) -> (Self) -> T {
    var result: (Self) -> T

    if let match: (Self) -> T = registry.get(){
      result = match
    } else {
      result = generator
